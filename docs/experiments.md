...
- Batch Size: 16
- Image Size: 640
- 参数量: 3.09M (与BiFPN相同，SimAM零参数)

**训练结果**:
| 指标 | 基线 | +BiFPN | +BiFPN+CBAM | +BiFPN+SimAM |
|------|------|--------|-------------|--------------|
| mAP50 | 0.893 | **0.912** | 0.839 | 0.842 |
| mAP50-95 | 0.595 | **0.600** | 0.508 | 0.512 |
| Precision | 0.880 | 0.873 | 0.826 | 0.801 |
| Recall | 0.850 | **0.882** | 0.782 | 0.785 |
| 推理速度 | 9ms | 1.4ms | - | 1.4ms |

**分析**:
- ❌ SimAM效果不如预期，mAP50=0.842（低于BiFPN的0.912）
- 比CBAM(0.839)略好，但仍显著低于纯BiFPN
- 零参数优势：推理速度与BiFPN相同(1.4ms)
- **结论**: 在BiFPN基础上添加注意力机制（无论CBAM还是SimAM）都会降低性能
- **原因分析**: BiFPN的加权特征融合已经足够有效，额外的注意力机制反而干扰了特征表达

**模型文件**: `runs/detect/bifpn_simam_v1/weights/best.pt`
...
| + BiFPN + SimAM | 0.842 | 0.512 | ~714 | 3.09M |
| + TensorRT FP16 | - | - | 待测 | - |

**最佳模型**: YOLOv8n + BiFPN (`models/bifpn_best.pt`)

---

## 改进路线图 (Roadmap)

### Phase 1: 数据优化 [优先级: 高] ⭐ 推荐下一步
- [ ] 修复235张问题图片的标注
- [ ] 收集/合成passenger类数据（当前缺失）
- [ ] 启用Copy-Paste增强（小目标提升）

### Phase 2: 架构优化 [优先级: 中]
- [x] BiFPN特征融合 ✓ (+1.9% mAP) **最佳改进**
- [x] CBAM注意力机制 ✗ (-7.3% mAP) 失败
- [x] SimAM注意力机制 ✗ (-7.0% mAP) 失败
- [ ] 高分辨率训练 (imgsz=960)
