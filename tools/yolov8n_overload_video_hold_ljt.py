...
class AssociationByteTracker:
    def __init__(
        ...
        hold_recover_center_dist: float,
        hold_recover_size_ratio: float,
    ):
        try:
            from boxmot import BYTETracker
        except ImportError as exc:
        ...
        self.hold_recover_center_dist = hold_recover_center_dist
        self.hold_recover_size_ratio = hold_recover_size_ratio
        self.frame_index = 0
        ...
        self.last_recovered_from_hold: Dict[str, int] = {}
    ...
    def update_scene(
        ...
        self._update_vehicle_hold_states({obj.stable_id for obj in tracked_vehicles.values()}, frame.shape)
    ...
    def _track_objects(self, kind: str, tracker, detections: List[Detection], frame) -> Dict[int, TrackedDetection]:
        ...
        for det_idx, raw_track_id in raw_matches.items():
            ...
            stable_id = self._stable_id_for(kind, raw_track_id, det, frame.shape)
    ...
    def _touch_stable(self, stable_id: str, det: Detection, kind: Optional[str] = None) -> None:
        ...
        if prev is not None:
            ...
            if move <= self.hold_move_thresh or norm_move <= self.hold_move_norm_thresh:
                ...
                stationary_frames = 0
            state = "stationary" if stationary_frames >= self.hold_stationary_min_frames else "active"
            held_until = int(prev.get("held_until", 0))
    ...
    def _find_hold_recovery_id(self, kind: str, det: Detection, frame_shape) -> Optional[str]:
        if kind != "vehicle":
            return None
        candidates: List[Tuple[float, str]] = []
        for stable_id, state in self.stable_objects.items():
            if state["kind"] != "vehicle" or state.get("state") != "held_lost":
                continue
            if self.frame_index > int(state.get("held_until", 0)):
                continue
            old_box = state["bbox"]
            center_dist = self._center_distance(old_box, det.bbox)
            if center_dist > self.hold_recover_center_dist:
                continue
            recover_iou = iou(old_box, det.bbox)
            if recover_iou < self.hold_recover_iou and center_dist > self.hold_recover_center_dist * 0.5:
                continue
            size_similarity = self._size_similarity(old_box, det.bbox)
            if size_similarity < self.hold_recover_size_ratio:
                continue
            score = recover_iou + max(0.0, 1.0 - center_dist / max(self.hold_recover_center_dist, 1.0)) + size_similarity * 0.25
    ...
    def _update_vehicle_hold_states(self, current_vehicle_ids: set[str], frame_shape) -> None:
        for stable_id, state in list(self.stable_objects.items()):
            if state["kind"] != "vehicle":
                continue
            if stable_id in current_vehicle_ids:
                continue
            if state.get("state") == "held_lost":
                if self.frame_index > int(state.get("held_until", 0)):
                    state["state"] = "expired"
                continue
            missed = self.frame_index - int(state["last_seen"])
    ...
    def hold_state(self, stable_id: int | str) -> str:
        state = self.stable_objects.get(str(stable_id))
        return str(state.get("state", "")) if state else ""

    def stationary_frames(self, stable_id: int | str) -> int:
        state = self.stable_objects.get(str(stable_id))
        return int(state.get("stationary_frames", 0)) if state else 0

    def recovered_from_hold(self, stable_id: int | str) -> bool:
        return self.last_recovered_from_hold.get(str(stable_id)) == self.frame_index

    def _near_frame_edge(self, box: List[float], frame_shape) -> bool:
        if frame_shape is None:
            return False
        height, width = frame_shape[:2]
        x1, y1, x2, y2 = box
        return (
            x1 <= self.hold_edge_margin
            or y1 <= self.hold_edge_margin
            or x2 >= width - self.hold_edge_margin
            or y2 >= height - self.hold_edge_margin
        )
    ...
    @staticmethod
    def _size_similarity(box_a: List[float], box_b: List[float]) -> float:
...
def create_vehicle_tracker(args):
    if args.tracker == "association":
        tracker = AssociationByteTracker(
            ...
            hold_recover_size_ratio=args.hold_recover_size_ratio,
        )
...
def main():
    ...
    parser.add_argument("--out", default="data/model_compare_video_vis/yolov8n-overload-hold-ljt.mp4")
    ...
    parser.add_argument("--hold-recover-center-dist", type=float, default=90.0)
    parser.add_argument("--hold-recover-size-ratio", type=float, default=0.45)
