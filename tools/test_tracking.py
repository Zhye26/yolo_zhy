...
def build_detector(args):
    ...
    if args.sam3:
        ...
        print(f"SAM3 sequential loading: {args.sam3_sequential}")
...
def test_tracking(video_path, args, output_path=None):
    ...
    print(f"Pipeline: {'YOLO+SAM3' if args.sam3 else 'YOLO'}")
...
def main():
    ...
    parser.add_argument("--task", choices=["all", "helmet", "passenger"], default="all", help="Violation task mode")
    parser.add_argument("--sam3", action="store_true", help="Enable SAM3 cascade")
    parser.add_argument("--sam3-checkpoint", default="/home/ubuntu/SAM3/sam3.pt", help="SAM3 checkpoint path")
    parser.add_argument("--sam3-alpha", type=float, default=0.35, help="SAM3 mask alpha")
    parser.add_argument("--sam3-sequential", action="store_true", help="Load SAM3 on demand")
